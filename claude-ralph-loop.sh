...
banner() {
    ...
    echo "  / ___|| | __ _ _   _  __| | ___  |  _ \\ __ _ | |_ __ _ | |__   | |    ___   ___  _ __"
    echo " | |    | |/ _\` | | | |/ _\` |/ _ \\ | |_) / _\` || | '_ \` || '_ \\  | |   / _ \\ / _ \\| '_ \\"
    echo " | |___ | | (_| | |_| | (_| |  __/ |  _ < (_| || | |_) || | | | | |__| (_) | (_) | |_) |"
    echo "  \\____||_|\\__,_|\\__,_|\\__,_|\\___| |_| \\_\\__,_||_| .__/ |_| |_| |_____\\___/ \\___/| .__/"
    echo "                                                 |_|                             |_|"
    echo -e "${NC}"
...
usage() {
    echo "Usage: claude-ralph-loop [OPTIONS]"
    echo ""
    ...
    echo "  --verbose           Show detailed Claude output"
    echo "  --help              Show this help message"
    echo ""
    echo "Examples:"
    echo "  claude-ralph-loop                # Process all pending stories"
    echo "  claude-ralph-loop --dry-run      # Preview what would happen"
    echo "  claude-ralph-loop --epic 1       # Process only Epic 1 stories"
    echo "  claude-ralph-loop --story 1-2    # Process only story 1-2"
    echo ""
    ...
    if [[ ! -f "$SPRINT_STATUS" ]]; then
        ...
        case "$choice" in
            1)
                ...
                run_claude_workflow "SM" "sprint-planning" "Initialize sprint status"
...
run_claude_workflow() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log WARN "[DRY-RUN] Would execute: claude with /$agent -> $workflow"
        return 0
    fi
...
process_story() {
    ...
    if [[ "$current_status" == "backlog" ]]; then
        ...
        run_claude_workflow "SM" "create-story" "Create story file for $story_key" "The story to create is $story_key from Epic $epic_num."
    ...
    if [[ "$current_status" == "ready-for-dev" ]]; then
        ...
        run_claude_workflow "DEV" "dev-story" "Implement story $story_key" "The story to implement is $story_key."
    ...
    if [[ "$current_status" == "review" && "$SKIP_CODE_REVIEW" == "false" ]]; then
        ...
        run_claude_workflow "DEV" "code-review" "Review implementation for story $story_key" "Review the changes made for story $story_key."
...
check_epic_completion() {
    ...
    if [[ "$pending" -eq 0 ]]; then
        ...
        if [[ "$run_retro" =~ ^[Yy] ]]; then
            run_claude_workflow "SM" "retrospective" "Run retrospective for Epic $epic_num"
